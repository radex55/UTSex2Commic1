...
/*
  ...
  $Id: coupon_admin.php,v 0.01 2014/03/10 17:56:34 Melanie Shepherd
...
?>
      ...
      <tr>
      <td>
<?php echo tep_draw_form('coupon', FILENAME_COUPON_ADMIN, 'action=update_confirm&oldaction=' . $HTTP_GET_VARS['oldaction'] . '&cid=' . $HTTP_GET_VARS['cid']); ?>
...
?>
      ...
      <td>
<?php 
    echo tep_draw_form('coupon', FILENAME_COUPON_ADMIN, 'action=update&oldaction='.$HTTP_GET_VARS['action'] . '&cid=' . $HTTP_GET_VARS['cid']); 
?>
      <table border="0" width="100%" cellspacing="0" cellpadding="6">
<?php
        $languages = tep_get_languages();
...
?>
        <td align="left" class="main"><?php echo COUPON_STARTDATE; ?></td>
        <td align="left"><?php echo tep_draw_date_selector('coupon_startdate', time(0,0,0, $coupon_startdate[1], $coupon_startdate[2], $coupon_startdate[0], 0)); ?></td>
        <td align="left" class="main"><?php echo COUPON_STARTDATE_HELP; ?></td>
      </tr>
      <tr>
        <td align="left" class="main"><?php echo COUPON_FINISHDATE; ?></td>
        <td align="left"><?php echo tep_draw_date_selector('coupon_finishdate', time(0,0,0, $coupon_finishdate[1], $coupon_finishdate[2], $coupon_finishdate[0], 0)); ?></td>
...
?>
          <tr>
            <td colspan="5" align="right" class="smallText"><?php echo '<a href="' . tep_href_link(FILENAME_COUPON_ADMIN, 'page=' . $HTTP_GET_VARS['page'] . '&cID=' . $cInfo->coupon_id . '&action=new') . '">' . tep_image_button('button_insert.gif', IMAGE_INSERT) . '</a>'; ?></td>
          </tr>
        </table></td>
    ...
    default:
      ...
      if ($HTTP_GET_VARS['action'] == 'voucherdelete') {
        $contents[] = array('text'=> TEXT_CONFIRM_DELETE . '</br></br>' . 
                '<a href="'.tep_href_link(FILENAME_COUPON_ADMIN,'action=confirmdelete&cid='.$HTTP_GET_VARS['cid'],'NONSSL').'">'.tep_image_button('button_confirm.gif','Confirm Delete Voucher').'</a>' .
